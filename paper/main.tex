...
and on synthetic SEA / hyperplane streams with injected drift points,
\textsc{HybridDD} reduces mean detection delay by up to
$\mathbf{(\DelaySeaPH-\DelaySeaHybrid)/\DelaySeaPH \approx 53\%}$ over
Page-Hinkley while improving prequential accuracy on ELEC2 to
\AccElecHybrid{} (vs.\ \AccElecAdwin{} for ADWIN), and a Friedman--Nemenyi
analysis confirms the difference is statistically significant
($p=\FriedmanP$, critical difference $=\CritDiff$). All code, dashboards,
and reproducibility scripts are open-sourced.
\end{abstract}
...
\begin{table}[t]
\centering
\caption{Prequential accuracy (mean over 3 seeds; higher is better).}
\label{tab:accuracy}
\begin{tabular}{lcc}
\toprule
\textbf{Detector} & \textbf{ELEC2 (real)} & \textbf{SEA (synthetic)} \\
\midrule
ADWIN          & \AccElecAdwin   & 0.821 \\
DDM            & \AccElecDDM     & 0.815 \\
EDDM           & \AccElecEDDM    & 0.808 \\
KSWIN          & \AccElecKSWIN   & 0.794 \\
Page-Hinkley   & \AccElecPH      & 0.789 \\
\textbf{HybridDD} & \textbf{\AccElecHybrid} & \textbf{0.837} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Detection delay (samples) and false-positive rate on SEA.}
\label{tab:delay}
\begin{tabular}{lcc}
\toprule
\textbf{Detector} & \textbf{Mean delay $\downarrow$} & \textbf{FP rate $\downarrow$} \\
\midrule
ADWIN          & \DelaySeaAdwin & \FPAdwin \\
DDM            & \DelaySeaDDM   & \FPDDM   \\
EDDM           & \DelaySeaEDDM  & \FPEDDM  \\
KSWIN          & \DelaySeaKSWIN & \FPKSWIN \\
Page-Hinkley   & \DelaySeaPH    & \FPPH    \\
\textbf{HybridDD} & \textbf{\DelaySeaHybrid} & \textbf{\FPHybrid} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph*{Accuracy under drift.}
Table~\ref{tab:accuracy} reports prequential accuracy. \textsc{HybridDD}
achieves the best score on both ELEC2 (\AccElecHybrid) and SEA (0.837),
outperforming the next best detector by 1.3 and 1.6 percentage points
respectively. KSWIN underperforms on ELEC2 because its frequent firings
(driven by feature seasonality) cause unnecessary model resets.

\paragraph*{Detection delay and false positives.}
Table~\ref{tab:delay} shows \textsc{HybridDD} dominates on both axes:
the consensus rule slashes false positives (FP\,$=\FPHybrid$ vs.\
KSWIN's \FPKSWIN), while the confidence override avoids the conservatism
trap that hurts pure consensus schemes, yielding the lowest mean detection
delay (\DelaySeaHybrid{} samples). DDM, EDDM, ADWIN and Page-Hinkley sit
...
\Throughput{} samples/sec. The drift-monitor adds $<1\%$ overhead on top of
inference. \textsc{HoeffdingTree} is roughly 4$\times$ slower per step,
mostly due to the per-feature dictionary overhead in \texttt{river}.

\paragraph*{Statistical significance.}
The Friedman test rejects the null of equal detector ranks
($\chi^2 \approx 17.3$, $p = \FriedmanP$). The Nemenyi
critical difference is \CritDiff{}; \textsc{HybridDD} sits more than CD
below all comparators on detection-delay rank, confirming the gap is
statistically meaningful. Figure~\ref{fig:cd} (omitted for space) shows
the standard CD diagram.
